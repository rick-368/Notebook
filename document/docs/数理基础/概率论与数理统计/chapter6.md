...
$$
\tilde{X}=\bold X = (X_1,X_2,...,X_n)
$$
样本中包含的个体数目n称为样本容量。

每一个样本$X_i$都是随机变量（其维数与总体一致）。若总体是一维的，则容量为$n$的样本可以看作$n$维的随机向量。但是，一旦取定一组样本，得到的是$n$个具体的$\bold x = (x_1,x_2,...,x_n)$，称此为样本的一次观察值，简称样本值或仍简称为样本。
#### 简单随机样本
设$\bold X = (X_1,X_2,...,X_n)$是来自总体$X$的随机样本，若满足以下两个性质：
1. 代表性：$X_1,X_2,...,X_n$中每一个与所考察的总体$X$有相同的分布。
2. 独立性：$X_1,X_2,...,X_n$是相互独立的随机变量。

则称$\bold X$为取自总体$X$的简单随机样本。  
获得简单随机样本的抽样方法被称为简单随机抽样。

由概率论知识，若总体$X$具有分布函数$F(x)$，则简单随机样本$\bold X$具有联合分布函数：
$$
F(\bold x) = \prod_{i=1}^n F(x_i)
$$
若总体$X$是连续型（离散型）随机变量，其概率密度函数（或分布律）为$f(x)$，则简单随机样本$\bold X$具有联合概率密度函数（或联合分布律）。
### 统计量
设$\bold X = (X_1,X_2,...,X_n)$是来自总体$X$的一个简单随机样本，若$g(\bold X)=g(X_1,X_2,...,X_n)$是样本$\bold X$上的函数，若$g$中不含任何未知的参数，则称$g(\bold X)$为样本$\bold X$的统计量。

说明：
1. 统计量可以仅从解析式上判断
2. 统计量仍然为随机变量。
3. 统计量的分布（称为抽样分布）一般与总体分布有关，即可以依赖未知参数。
4. 若$\bold x = (x_1,x_2,...,x_n)$是样本$\bold X$的一次观察值，则$g(\bold x)$是样本$g(\bold X)$的观察值，称之为统计量的值。

#### 常用统计量
设$\bold X = (X_1,X_2,...,X_n)$是来自总体$X$的一个样本。
1. 样本均值：
$$
