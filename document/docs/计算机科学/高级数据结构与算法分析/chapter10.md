...
除了上述两种方法，我们可以通过两两比较的方式求解，我们构造另一个集合B，$b_1,b_2,\cdots,b_n$，然后执行下列算法：
```
for 1<=i<=n:
    B[i]=0
...
这个算法$W(n)=\sqrt{n}W(\sqrt{n})+O(\sqrt{n}^2)$，求得$W(n)=O(n\log \log n)$。$D(n)=D(\sqrt{n})+O(1)$，求得$D(n)=O(\log \log n)$。
